...
# local import
from . import io
from .io import get_shared_data
...
def convertToStdMTG(g):
    """
    Copy `g` into a new mtg converting 'position' triplet properties into XX,YY,ZZ
    """
...
def createEdgeRepresentation(begnode,endnode,positions,material, translation = None):
    if begnode is None or endnode is None: 
        ...
        return None
    res = pgl.Polyline([positions[begnode], positions[endnode]],width=1)
    #res = pgl.Group([res,pgl.Translated((positions[begnode]+positions[endnode])/2,pgl.Text(str(endnode)))])
    if translation:
        res = pgl.Translated(translation, res)
    return pgl.Shape(res,material,endnode)
...
def createAttractorsRepresentation(attractors, width, material):
    pointset = pgl.PointSet(attractors)
    pointset.width = width
    return pgl.Scene([pgl.Shape(pointset, material)])


def getConeDirections(u, angle):
    return ConePerception().getConeAxis(u, angle)  ## _pointreconstruction

def createCone(p1, p2, angle, resolution=20):
    from math import tan
    o = pgl.Vector3(p2)
    nz = pgl.Vector3(p1) - pgl.Vector3(p2)
    h = nz.normalize()
    ny = pgl.cross(nz,pgl.Vector3.OX)
    if  pgl.norm(ny) < 1e-3:
        ny = pgl.cross(nz,pgl.Vector3.OY)
    ny.normalize()
    nx = pgl.cross(ny,nz)
    nx.normalize()
    radius = tan(angle/2)*h
    return pgl.Translated(o,pgl.Oriented(nx,ny,pgl.Cone(radius,h,slices=resolution)))
...
def createConeRepresentation(p1, axises, height, angle, material):
    ...
    for p2 in p2list:
        ...
        res += pgl.Scene([pgl.Shape(c, material)])
...
class GLMTGEditor(QGLViewer):
    ...
    def __init__(self,parent=None,pointfile = None, mtgfile = None):
        if parent:
            QGLViewer.__init__(self,parent=None)
        else:
        ...
        self.pointMaterial = pgl.Material(self.theme['Points'],1)
        self.contractedpointMaterial = pgl.Material(self.theme['ContractedPoints'],1)
        self.ctrlPointColor = self.theme['CtrlPoints']
        self.newCtrlPointColor = self.theme['NewCtrlPoints']
        self.edgeInfMaterial = pgl.Material(self.theme['EdgeInf'],1)
        self.edgePlusMaterial = pgl.Material(self.theme['EdgePlus'],1)
        self.selectedPointColor = pgl.Material(self.theme['SelectedCtrlPoints'],1)
        self.modelMaterial =  pgl.Material(self.theme['3DModel'],1)
        ...
        self.discretizer  = pgl.Discretizer()
        self.glrenderer   = pgl.GLRenderer(self.discretizer)
        self.glrenderer.renderingMode = pgl.GLRenderer.Dynamic
        self.ctrlrenderer = pgl.GLCtrlPointRenderer(self.discretizer)
        try:
        ...
        self.points = None if pointfile is None else pgl.Scene(pointfile)[0].geometry
        ...
        self.attractorsMaterial = pgl.Material(self.theme['LocalAttractors'],1)
        self.attractorsDisplay = True
        self.attractors = None
        self.attractorsRep = pgl.Scene() 
        
        self.coneMaterial = pgl.Material(ambient=self.theme['Cone'], diffuse=1, transparency=0.5)
        self.conesDisplay = True
        self.cones = None
        self.conesRep = pgl.Scene()
    ...
    def createBackup(self):
        ...
        self.emit(QtCore.SIGNAL('undoAvailable(bool)'),True)
    ...
    def undo(self):
        if len(self.backupmtg) > 0:
            ...
            self.updateGL()
            if len(self.backupmtg) > 0:
                self.emit(QtCore.SIGNAL('undoAvailable(bool)'),False)
        else:
    ...
    def setclassicdata(self,dataedit, dataoriginal, pointname = None):
        firstname = get_shared_data('mtgdata','edition', dataedit+'.bmtg')
        if os.path.exists(firstname): 
            mtgname = firstname
        else:
            mtgname = get_shared_data('mtgdata',dataoriginal+'.bmtg')
        ...
        if pointname is None: 
            pointname = dataedit
        pointfname = get_shared_data('pointset', pointname+'.bgeom')
        print pointfname ##
        self.readPoints(pointfname)

    def puu1(self):
        self.setclassicdata('puu1','puu1_originalref')
        
    def puu3(self):
        self.setclassicdata('puu3','puu3_originalref')
        self.reorient()
        
    def cherry(self):
        self.setclassicdata('cherry','cherry_originalref','cherry_200k')
        
        
    def openMTG(self):
        initialname = os.path.dirname(self.mtgfile) if self.mtgfile else get_shared_data('mtgdata')
        fname = QtGui.QFileDialog.getOpenFileName(self, "Open MTG file",
                                                initialname,
    ...
    def setMTG(self,mtg,fname):
        ...
        self.mtgfile = fname
        self.mtgrep, self.mtgrepindex  = createMTGRepresentation(self.mtg,self.edgeInfMaterial,self.edgePlusMaterial)        
        self.adjustTo(self.mtgrep)
        pointsize = self.getCtrlPointSize()
        self.ctrlPointPrimitive = pgl.Sphere(pointsize)
        self.ctrlPoints = createCtrlPoints(self.mtg,self.ctrlPointColor,self.propertyposition,self.__update_value__)
        self.createCtrlPointRepresentation()
    ...
    def saveMTG(self):
        initialname = os.path.dirname(self.mtgfile) if self.mtgfile else get_shared_data('mtgdata')
        fname = QtGui.QFileDialog.getSaveFileName(self, "Save MTG file",
                                                initialname,
                                                "MTG Files (*.mtg;*.bmtg);;All Files (*.*)")
        if not fname: return
        fname = str(fname)
        self.write_mtg(fname)
        
    def write_mtg(self,fname):
        fname = str(fname)
        import os.path,shutil
        if os.path.exists(fname):
            shutil.copy(fname,fname+'~')
        if os.path.splitext(fname)[1] == '.bmtg':
           io.writefile(fname,self.mtg)
        else: # .mtg
            # readable mtg format from openalea.mtg module
            stdmtg = convertToStdMTG(self.mtg)
            io.write_mtg_file(fname, stdmtg, properties=[('XX','REAL'), ('YY','REAL'), ('ZZ','REAL'), ('radius','REAL')])
        self.mtgfile = fname
    ...
    def adjustTo(self,obj):
        ...
        bbx = bbc.boundingbox
        print type(bbx)
        #print bbx.getSize(), pgl.norm(bbx.getSize())
    ...
    def keyPressEvent(self, event):
        ...
        mousepos = QtGui.QCursor.pos() - globalpos
    ...
    def mousePressEvent(self,event):
        ...
        if self.mode == self.Selection:
            ...
            if cCtrlPoint:
                ...
                QtGui.QMessageBox.warning(self,'Selection','Cannot find a node to select')
    ...
    def newChild(self):
        ...
        if nbchild == 0:
            ...
            ipos = positions[nid]
            dirl = pgl.direction(l)
            dirView = toV3(self.camera().viewDirection())
            nbcandidates = 10
            candidates = [ipos + pgl.Matrix3.axisRotation(dirView,ang*2*math.pi/nbcandidates)*l for ang in xrange(nbcandidates)]
            ...
            siblingpos = [ipos+norml*pgl.direction(positions[sib]-ipos) for sib in siblings]
            # point set at a given distance criteria
            factor1 = [abs(pgl.norm(c-ipos) - norml) for c in candidates]
            # distance to other nodes criteria
            factor2 = [sum([pgl.norm(pos-c) for pos in siblingpos]) for c in candidates]
            max1, max2 = max(factor1), max(factor2)
            cmplist = [(i,(factor1[i]/max1)+2*(1-(factor2[i]/max2))) for i in xrange(nbcandidates)]
            # self.setTempInfoDisplay(pgl.Scene([pgl.Shape(pgl.PointSet(ocandidates,width = self.pointWidth+2),pgl.Material((255,0,255))),
                                    # pgl.Shape(pgl.PointSet(candidates,width = self.pointWidth+2),pgl.Material((255,255,0))),
                                    # pgl.Shape(pgl.PointSet(siblingpos,width = self.pointWidth+8),pgl.Material((255,255,255))),
                                    # pgl.Shape(pgl.Group([pgl.Translated(ocandidates[i],pgl.Text(str(int(100*cmplist[i][1])))) for i in xrange(len(candidates))]),pgl.Material((255,255,0)))
                                    # ]))
