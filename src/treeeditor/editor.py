...
class TreeEditorWidget(_QGLViewer, _AbstractEditor):
    ...
    def __init__(self, parent=None, tree='default', background='default', theme=None, **presenters):
        ...
        # super __init__
        if parent:
            _QGLViewer.__init__(self, parent=None)
        else:
            _QGLViewer.__init__(self)

        if theme is None:
            from .material import THEME as theme
    ...
    def enabledClippingPlane(self, enabled):
        ...
        if enabled:
            self.show_message('Enabled Clipping Plane')
        else:
            self.show_message('Disabled Clipping Plane')
        if self.isVisible():
            self.updateGL()
    ...
    def menu_actions(self):
        actions = [['', a[0], a[1], a[2]] for a in self.get_plugin_actions()]
        return actions
    ...
    def _create_menu(self, title, actions, filter_dict=None):
        ...
        for action in actions:
            if action is None:
                ...
                menu.addAction(self._get_action(**action))
        menu.addSeparator()
    ...
    def _mouse_string(self, event):
        ...
        if hasattr(event, 'button'):
            button = event.button()
            if QtCore.Qt.LeftButton == button:
                buttons += 'Left'
            elif QtCore.Qt.RightButton == button:
                buttons += 'Right'
            elif QtCore.Qt.MidButton == button:
                buttons += 'Middle'
